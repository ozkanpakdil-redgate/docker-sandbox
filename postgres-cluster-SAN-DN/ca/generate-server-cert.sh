...
if [ -f "$CA_DIR/redgatemonitor-nopass.pfx" ]; then
...
chown postgres:postgres *.crt *.key *.pfx 2>/dev/null || true
chmod 600 server.key redgatemonitor.key ca.key redgatemonitor.pfx 2>/dev/null || true
if [ -f "redgatemonitor-nopass.pfx" ]; then
    chmod 600 redgatemonitor-nopass.pfx 2>/dev/null || true
fi
